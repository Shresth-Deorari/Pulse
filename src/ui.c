...
void ui_handle_input(int ch, int num_processes) {
    ...
    if (drawable_height < 1) drawable_height = 1;
    ...
    switch (ch) {
        ...
        case KEY_DOWN: if (scroll_offset < max_scroll) scroll_offset++; break;
    }
}
...
void draw_header(void) {
    ...
    mvwprintw(header_win, 0, 1, "Pulse - Resource Monitor (Scroll with Arrow Keys/Mouse Wheel)");
...
void draw_cpu_panel(const double* cpu_usage, int num_total_cpu_entries) {
    ...
    int num_cols = (panel_width > 2) ? (panel_width - 2) / CPU_ITEM_FIXED_WIDTH : 1;
    ...
    for (int i = 0; i < num_total_cpu_entries; ++i) {
        ...
        (i == 0) ? snprintf(buffer, sizeof(buffer), "Aggr: %.1f%%", cpu_usage[i])
                 : snprintf(buffer, sizeof(buffer), "CPU%d: %.1f%%", i - 1, cpu_usage[i]);
...
void draw_mem_panel(const memStats* mem_info) {
    ...
    unsigned long mem_used = mem_info->memTotal > 0 ? mem_info->memTotal - mem_info->memAvailable : 0;
    double mem_percent = mem_info->memTotal > 0 ? 100.0 * mem_used / mem_info->memTotal : 0.0;
    
    unsigned long swap_used = mem_info->swapTotal > 0 ? mem_info->swapTotal - mem_info->swapFree : 0;
    double swap_percent = mem_info->swapTotal > 0 ? 100.0 * swap_used / mem_info->swapTotal : 0.0;

    char mem_str[64], swap_str[64];
    snprintf(mem_str, sizeof(mem_str), "Mem: %lu/%lu KB (%.1f%%)", mem_used, mem_info->memTotal, mem_percent);
    snprintf(swap_str, sizeof(swap_str), "Swap: %lu/%lu KB (%.1f%%)", swap_used, mem_info->swapTotal, swap_percent);
...
void draw_process_panel(const ProcessInfo* processes, int num_processes) {
    ...
    if (drawable_height < 1) return;

    wattron(proc_win, COLOR_PAIR(PROC_HEADER_PAIR));
    mvwprintw(proc_win, 1, 1, "%-6s %-20s %-5s %-6s %-6s %-10s %-10s", "PID", "COMMAND", "S", "CPU%", "MEM%", "VIRT", "RES");
    for (int x = 80; x < width - 1; ++x) mvwaddch(proc_win, 1, x, ' ');
    wattroff(proc_win, COLOR_PAIR(PROC_HEADER_PAIR));

    for (int i = 0; i < drawable_height; ++i) {
        int proc_index = scroll_offset + i;
        if (proc_index >= num_processes) break;
