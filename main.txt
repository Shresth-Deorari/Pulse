...
void* data_collector_thread(void* arg) {
    ...
    while (running) {
        ...
        for (int i = 0; i < curr_procs.count; ++i) {
            ...
            // Note: O(n*m) process lookup to calculate CPU delta
...
ProcessList get_all_processes() {
    ...
    if (!proc_dir) return (ProcessList){.items = NULL, .count = 0};

    ProcessList list = {.items = malloc(sizeof(pidStats) * MAX_PROCESSES), .count = 0};
    if (!list.items) {
        closedir(proc_dir);
    ...
    while ((entry = readdir(proc_dir)) != NULL && list.count < MAX_PROCESSES) {
        ...
        if (file_contents) {
            if (pidParser(file_contents, &list.items[list.count])) list.count++;
            free(file_contents);
        }
    }
    closedir(proc_dir);
...
char *read_file_dynamically(const char *path) {
    ...
    size_t total_read = 0;
    while (1) {
        ssize_t bytes_read = read(fd, buffer + total_read, buffer_size - total_read - 1);
        if (bytes_read > 0) {
